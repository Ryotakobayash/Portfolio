...
date: "2026-05-02"
excerpt: "Astroベースのポートフォリオサイトに、Three.jsを使ったASCIIアートの3D装飾を導入しました。結論となる実装手順を最優先で紹介し、後半で技術選定の理由やその他の選択肢についてまとめます。"
tags:
  - Astro
  - React
  - Three.js
...
今回の実装にあたり、最初からR3Fの正解に辿り着いたわけではなく、Geminiと壁打ちしながら手法を絞り込んでいきました。ご参考までに、用途によっては使えるかもしれない「不採用になった選択肢」も載せておきます。
...
この記事を書きつつ振り返ってみて、「Reactを挟まないVanilla JS + `AsciiEffect` の方がAstroの思想に忠実というか…スマートでよかったのでは？」と思いました。こう納得した、というメモも残します。
...
「Astroの軽量さ」を本気で守るなら、クライアントサイドでの3Dレンダリング自体を避けて、ビルド時にヘッドレスでレンダリングした静的ASCIIアートをCSSアニメーションで切り替える…みたいなアプローチになりますが、背景の装飾にそこまでやるかというと、やりすぎだと思います。
