...
**背景・目的:**
- `/talks` ページに今後の登壇予定や過去の登壇履歴（スライド）を追加する際、`src/data/talks.json` を編集するだけで済むような仕組みを作った。
- 将来、新しい登壇情報を追加する際に迷わないよう、README または専用のドキュメントにその追加手順（JSONのスキーマやSpeaker Deckの埋め込みIDの取得方法など）を明記しておきたい。

**要件・仕様:**
- [ ] `README.md` もしくは `docs/` ディレクトリ内に、`/talks` へのデータ追加手順を記載したセクション・ファイルを作成する。
- [ ] `upcoming` と `past` それぞれのオブジェクト構造、必須プロパティ、`embedUrl` の仕様を説明する。

**完了条件 (Acceptance Criteria):**
- [ ] 登壇情報の追加手順がドキュメント化されていること
