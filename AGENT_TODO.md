...
### [タスク名: テキストリンクホバー時のリンクプレビュー]
**背景・目的:**
- 記事本文内の内部リンク（`/posts/xxx`）にマウスホバーした際、リンク先記事のタイトルや日付をツールチップ風に表示し、コンテンツ間のナビゲーションを改善する。

**要件・仕様:**
- [ ] `[slug].astro` の `<script>` ブロックに `setupLinkPreviews()` 関数を追加する
- [ ] 記事メタデータは `define:vars` でビルド時に埋め込む方式とする（fetch 不要）
- [ ] モバイル（`hover: none`）ではプレビューを無効化する
- [ ] プレビュー要素のスタイルを追加する

**関連ドキュメント:**
- 実装計画: `docs/plan-view-transitions-and-ux.md` の Phase 2 を参照

**完了条件 (Acceptance Criteria):**
- [ ] 記事内の内部リンクにホバーするとタイトル・日付のプレビューが表示されること
- [ ] モバイルではプレビューが表示されないこと
