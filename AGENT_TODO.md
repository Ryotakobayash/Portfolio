...
- Specifications:
  ...
  - [x] `:target` 疑似クラスおよびアニメーションを活用し、戻り先の数字が一目でわかるパルスハイライト演出を適用（約3秒で通常スタイルへ自然フェードアウト）
  - [x] 固定ヘッダーに数字や段落文頭が隠れないことを保証
  - [x] 記事末尾の脚注（`.footnotes`）テキスト色を `var(--color-text)` に変更し、ダークモードでも本文同等の高いコントラストと視認性を確保
- Acceptance Criteria:
  - [x] 脚注末尾の戻るボタン（↩）をクリックしたとき、本文の数字がヘッダーの下で隠れず、数行上の文章とともに視界内に収まること
  - [x] 戻り先の数字が視覚的にハイライトされ、どこに戻ったか迷わないこと
  - [x] パルスライトが約3秒で自然に元のスタイルへ戻ること
  - [x] ダークモード・ライトモード双方で脚注の文字が本文同様にクッキリ読みやすいコントラストであること
  - [x] デスクトップ・モバイル双方で自然なスクロール位置になること

---

### [TASK-014] CRTエフェクトの網羅的強化（画面湾曲・ベゼル・走査ビーム・ガラス反射・電源ON/OFF展開）
- Status: `DONE`
- Completed: 2026-09-15
- Priority: P1
- Target Files: `src/components/CRTOverlay.astro`, `src/styles/global.css`, `src/layouts/BaseLayout.astro`
- Verification Command: `pnpm build`
- User Context: FX / CRT の ON/OFF 切替機能が備わったことで、可読性を損なわずに存分にこだわったレトロモニター演出が可能になった。画面の物理的湾曲、四隅のベゼルフレーム、ビネット減光、曲面ガラスの環境光反射、上から下へ巡回する走査ビーム、周辺部の色収差、電源ON/OFF展開演出など、思いつく限りのリアルなCRT演出を一通り試作・実装し、後から好みに応じて取捨選択・強度調整できるようにする（本文を汚す砂嵐・ノイズは除外）。
- Specifications:
  - [x] `CRTOverlay.astro` をマルチレイヤー構成に拡張（ベゼル湾曲フレーム、曲面ビネット、ガラスツヤ反射、走査ビーム、RGBアパーチャーグリル）
  - [x] 画面外周部にブラウン管特有の内側カーブとインセットシャドウによる「画面の湾曲」を美しく再現
  - [x] 上から下へゆっくり（約8〜10秒周期で）通過する電子ビーム（走査ビーム）のスイープ演出を追加
  - [x] 画面外周部にのみ生じる光学的な色収差（RGBフリンジ）を付与（中央本文はシャープなまま）
  - [x] トグル切り替え時に、水平線からパッと画面が展開／中心に収束するブラウン管電源ON/OFFアニメーションを実装
  - [x] 各エフェクトの強度・透明度・周期を CSS カスタムプロパティ（CSS変数）で一元管理し、調整や個別無効化を容易にする
  - [x] `data-fx="off"` 時には全エフェクトが瞬時に完全非表示になり、パフォーマンスやアクセシビリティを100%維持
- Acceptance Criteria:
  - [x] `FX: ON` 時に、画面湾曲・ベゼル・ビネット・走査ビーム・ガラス反射が自然に重なり、ヴィンテージCRTの臨場感が得られること
  - [x] 本文の文字を汚す砂嵐やグリッチノイズがなく、記事やコードが快適に読めること
...
- User Context: 記事執筆頻度がたまにある程度であるため、画像の保管場所やMarkdown記法（図表自動ナンバリングなど）のルールを忘れやすい。コマンド1発でルール通りの雛形ファイルと画像フォルダを生成し、かつルールをドキュメント（README.md, docs/article-workflow.md, AGENTS.md）に体系化して人間・LLM双方から参照しやすくする。
- Specifications:
  ...
  - [x] `AGENTS.md` にブログ記事の作成ルール（命名規則、画像管理、コミット注意点など）を明記し、LLM協業時に確実に守られるようにする
- Acceptance Criteria:
  - [x] `pnpm new:post --help` が正常にヘルプを出力すること
  - [x] スクリプト実行により、指定した日付・タイトルの記事と画像フォルダが想定通りの構造で生成されること
  - [x] `pnpm build` がエラーなく通過すること
  - [x] README.md / docs/article-workflow.md / AGENTS.md に執筆・画像保管の仕組みが分かりやすく明記されていること
